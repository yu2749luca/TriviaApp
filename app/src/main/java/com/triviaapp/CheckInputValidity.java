...
public class CheckInputValidity {
    ...
    private boolean checkDrop(String input){
        ...
        if (message.contains(check)){checkDrop=false;}
    ...
    private boolean checkCompare (String input){
        ...
        for (int i =2;i<message.length()-1;i++){
            ...
            if(message.charAt(i)=='=') {
                ...
                if ((previous=='!')||(previous=='>')||(previous=='<')){checkSigns=true;}
                if (checkAfterDigit&&checkFrontDigit&&checkSigns){checkCompare=false;i=message.length();}
            }
